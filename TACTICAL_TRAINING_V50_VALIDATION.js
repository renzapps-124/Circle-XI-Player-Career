...
const checks=[
  ...
  ['tactical vision state chain',game.includes("['SCENARIO_SETUP','PRE_RECEIVE','SCAN_WINDOW','RECEIVE','DECISION','EXECUTION','OUTCOME','NEXT_SCENARIO']")],
  ...
  ['ball target is an intended receiver, not a teleport',game.includes("s.ball.intendedId=target===s.player?'user':target.id")&&game.includes("tacticalReleaseBall(s,source,target,'pass'")],
...
console.log(`\nTactical training v50 validation: ${passed}/${checks.length} checks passed`);
